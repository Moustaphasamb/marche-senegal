# 🇸🇳 MARCHÉ SÉNÉGAL — Briefing Projet pour les agents (Claude Code, Codex)
...
# Journal partagé Claude / Codex

Deux agents travaillent sur ce projet sans se voir. Leur point de rendez-vous est
`../docs/journal.md`, dans le dépôt de pilotage `ms`.

- **Avant de commencer :** `head -60 ../docs/journal.md`.
- **Dès qu'une tâche est terminée, en pause ou bloquée :** ajouter une entrée en
  haut du journal (format décrit dans le fichier), sur le dépôt `frontend`.
- **Ne jamais réécrire l'entrée d'un autre agent.**

Terminer chaque message de commit par `Agent: claude` ou `Agent: codex`.
